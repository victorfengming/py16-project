...
def myadmin_dologin(request):
    # 先检测验证码是否正确
    # 全部转大写,为了不区分大小写
    if request.session.get('verifycode').upper() != request.POST.get('verifycode').upper():
        ...
        if request.POST['username'] == 'admin' and request.POST['password'] == '123456':
            ...
            return HttpResponse('<script>alert("登录成功,欢迎使用");location.href="/myadmin/"</script>')
        else:
            return HttpResponse('<script>alert("用户名或者密码错误");location.href="/myadmin/login/"</script>')
